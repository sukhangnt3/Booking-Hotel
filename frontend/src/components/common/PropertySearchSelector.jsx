...
export default function PropertySearchSelector({
  ...
  const filteredHotels = useMemo(() => {
    ...
    return hotels.filter((h) => {
      ...
      // 🛑 ƯU TIÊN PROVINCE TRƯỚC
      const city = String(h.province || h.city || "").toLowerCase();
          ...
          <div className="max-h-60 overflow-y-auto p-2 space-y-1">
            ...
            {filteredHotels.length > 0 ? (
              filteredHotels.map((hotel) => {
                ...
                const hotelName =
                  hotel.name || hotel.hotelNameVi || "Khách sạn";

                // 🛑 ƯU TIÊN HIỂN THỊ TỈNH/THÀNH PHỐ THẬT (PROVINCE)
                const hotelCity = hotel.province || hotel.city || "Việt Nam";
