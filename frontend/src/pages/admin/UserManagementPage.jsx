          ...
          <tbody className="divide-y divide-slate-200">
            {users.map((u) => (
              <tr key={u.id}>
                ...
                <td className="p-4"><span className="px-2.5 py-1 bg-slate-100 font-bold text-xs rounded-md">{u.role}</span></td>
                <td className="p-4">
                  <span className={`px-2.5 py-1 text-xs font-bold rounded-full ${u.active ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-700"}`}>
                    {u.active ? "Hoạt động" : "Đã khóa"}
                  </span>
                </td>
                <td className="p-4 text-right">
                  <button onClick={() => toggleUser(u.id)} className={`px-3 py-1.5 font-bold text-xs rounded-lg text-white ${u.active ? "bg-rose-600" : "bg-emerald-600"}`}>
